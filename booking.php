    ...
    if (!$room_id || !$check_in) {
        ...
        if (!$room) {
            ...
            try {
                ...
                $error = 'Booking failed. Ensure project_extras.sql (booking table) is imported.';
            }
        }
    }
}
                ...
                <div class="form-group">
                    ...
                    <select name="room_id" required>
                        ...
                        <option value="<?= (int)$r['room_id'] ?>" <?= ($roomId && $roomId == $r['room_id']) ? 'selected' : '' ?>>
                            Room <?= htmlspecialchars($r['room_no']) ?> — <?= htmlspecialchars($r['h_name'] ?? '') ?> — ₹<?= number_format($r['price']) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
